...
public class BlockLinkChecker {
	 ...
	 */
	public static BlockLink canLink(Workspace workspace, Block block1,
			Block block2, BlockConnector con1, BlockConnector con2) {
		if (checkRules(block1, block2, con1, con2)) {
			return BlockLink
					.getBlockLink(workspace, block1, block2, con1, con2);
		}
	 ...
	 */
	public static BlockLink getLink(Workspace workspace,
			RenderableBlock rblock1, Iterable<RenderableBlock> otherBlocks) {
		...
		System.out.println("rblock1 is "+rblock1.getBlock().getGenusName());
		...
		for (RenderableBlock rblock2 : otherBlocks) {
			...
			System.out.println("rblock2 is "+rblock2.getBlock().getGenusName());
				...
				for (BlockConnector currentSocket : getSocketEquivalents(block2)) {
					...
					if (currentDistance < closestDistance) {
						...
						if(rblock1.getBlock().getGenusName().equals("function")&&rblock2.getBlock().getGenusName().equals("execution")){
							...
							workspace.notifyListeners(new WorkspaceEvent(workspace, rblock2
									.getParentWidget(), rblock2.getBlockID(),
		...
		return BlockLink.getBlockLink(workspace, block1, closestBlock2,
				closestSocket1, closestSocket2);
	}
	 ...
	 *         no such matching exists.
	 */
	public static BlockLink getWeakLink(Workspace workspace,
			RenderableBlock rblock1, Iterable<RenderableBlock> otherBlocks) {
		Block block1 = workspace.getEnv().getBlock(rblock1.getBlockID());
		BlockConnector closestSocket1 = null;
		BlockConnector closestSocket2 = null;
