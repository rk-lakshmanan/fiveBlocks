...
public class Translator
{
	...
	private Set<String> newFunctionNameSet;
	private Set<String> newFunctionDecSet;
	...
	public String genreateHeaderCommand()
	{
		...
		if(!newFunctionDecSet.isEmpty()){
			for(String functionDeclaration:newFunctionDecSet){
				headerCommand.append(functionDeclaration+"\n"	);
			}
			headerCommand.append("\n");
		}
	...
	public String translate(Long blockId) throws SocketNullException, SubroutineNotDeclaredException, BlockException
	{
		...
		if(block.getGenusName().equals("function")){
			return rootTranslatorBlock.getCode();
		}
	...
	public void reset()
	{
		newFunctionNameSet= new HashSet<String>();
		newFunctionDecSet= new HashSet<String>();
	...
	{
		if (functionName.equals("loop") ||functionName.equals("setup") || functionNameSet.contains(functionName)||newFunctionNameSet.contains(functionName))
		{
	...
	//new method for adding function name
	public void addFunctionName(Long blockId, String functionName,ArrayList<String> paramList,String returnParam) throws SubroutineNameDuplicatedException
	{
		if(functionName.equals("loop") ||functionName.equals("setup") || functionNameSet.contains(functionName)||newFunctionNameSet.contains(functionName))
		{
			throw new SubroutineNameDuplicatedException(blockId);
		}
		newFunctionNameSet.add(functionName);
		String returnType;
		if(returnParam!=null){
			returnType = "float ";
		}else{
		...
		for(int i = 0; i<paramList.size();i++){
			inputParams += "float ";
			inputParams += paramList.get(i);
			if(i!=paramList.size()-1){
				inputParams+=", ";
			}
		}
		newFunctionDecSet.add(returnType+functionName+"("+inputParams+");");
	}
