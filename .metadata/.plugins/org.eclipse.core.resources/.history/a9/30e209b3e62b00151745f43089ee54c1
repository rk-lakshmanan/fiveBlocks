	...
	public void actionPerformed(ActionEvent e)
	{
		...
		for (RenderableBlock renderableBlock:renderableBlocks)
		{
			...
			if (!block.hasPlug() && (Block.NULL.equals(block.getBeforeBlockID())))
			{
				...
				//added code for function
				if(block.getGenusName().equals("function")){
					
					
					//get the function name by accessing the block that it connected to
					Long id = block.getSocketAt(0).getBlockID();
					Block nameBlock =workspace.getEnv().getBlock(id);
					String name = nameBlock.getBlockLabel();
					
					//get list of input params
					ArrayList<String> paramList = new ArrayList<String>();
					int i = 1;
					while(block.getSocketAt(i).getBlockID()!=Block.NULL){
						paramList.add(workspace.getEnv().getBlock(block.getSocketAt(i).getBlockID()).getBlockLabel());
						i++;
					}
					i++;
					String returnParam;
					if(block.getSocketAt(i).getBlockID()!=Block.NULL){
						returnParam = workspace.getEnv().getBlock(block.getSocketAt(i).getBlockID()).getBlockLabel();
					}else{
						returnParam=null;
					}
					try
					{
						translator.addFunctionName(block.getBlockID(), name,paramList,returnParam);
					}
